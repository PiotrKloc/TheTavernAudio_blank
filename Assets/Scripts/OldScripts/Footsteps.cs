    ...
    void PlayFootsteps()
    {
        ...
        if (Physics.Raycast(transform.position, Vector3.down, out hit, distToGround + 0.5f))
        {
            if (hit.collider.CompareTag("Stone"))
            {
                ...
                FootstepsSound.setParameterByNameWithLabel("Foot-Swither", "Stone");
            ...
            else if (hit.collider.CompareTag("Wood"))
            {
                FootstepsSound = FMODUnity.RuntimeManager.CreateInstance(footstepsEvent);
                FootstepsSound.set3DAttributes(FMODUnity.RuntimeUtils.To3DAttributes(gameObject.transform));
                FootstepsSound.setParameterByNameWithLabel("Foot-Swither", "Wood");
                FootstepsSound.start();
                FootstepsSound.release();
            }
        ...
        if(IsGrounded())
        {
            ...
            if (IsGrounded())
            {
                ...
                if (Physics.Raycast(transform.position, Vector3.down, out hit, distToGround + 0.5f))
                {
                    ...
                    if (hit.collider.CompareTag("Stone"))
                    {
                        JumpSound.setParameterByNameWithLabel("Foot-Swither", "Stone");
                        JumpSound.start();
                    }
                    else if (hit.collider.CompareTag("Wood"))
                    {
                        JumpSound.setParameterByNameWithLabel("Foot-Swither", "Wood");
                        JumpSound.start();
                    }
                    else if (hit.collider.CompareTag("Inside_stone"))
                    {
                        LandSound.setParameterByNameWithLabel("Foot-Swither", "Stone");
                        JumpSound.start();
                    }
                    else if (hit.collider.CompareTag("Inside_wood"))
                    {
                        LandSound.setParameterByNameWithLabel("Foot-Swither", "Wood");
                        JumpSound.start();
                    }
                    else if (hit.collider.CompareTag("Bed"))
                    {
                        JumpSound.setParameterByNameWithLabel("Foot-Swither", "Bed");
                        JumpSound.start();
                    }
                    else
                    {
        ...
        if (IsGrounded() && isGrounded == false)
        {
            if (isJumping == true)
            {
                ...
                if (Physics.Raycast(transform.position, Vector3.down, out hit, distToGround + 0.5f))
                {
                    ...
                    {
                        LandSound.setParameterByNameWithLabel("Foot-Swither", "Bed");
                        LandSound.start();
                    }
                    else
                    {
